import { Fun } from "widgets-for-react"

export type ValidationResult = { kind:"too early to tell" } | { kind:"valid" } | { kind:"invalid", error:string }

export const validations = {
  tooEarlyToTell:({ kind:"too early to tell" }) as ValidationResult,
  valid:{ kind:"valid" } as ValidationResult,
  invalid:(error:string) => ({ kind:"invalid", error }) as ValidationResult
}

export type Validator<s> = Fun<s, ValidationResult>
export type Validations<t> = { [f in keyof t]?:ValidationResult }

export const emailRegex = () => /(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])/g
export const phoneNumberRegex = () => /^[+]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-\s\./0-9]*$/g
